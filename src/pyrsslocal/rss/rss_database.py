...
class DatabaseRSS (Database) :
    ...
    def enumerate_latest_status(self, postid, nb = 1, connect = True):
        ...
        sql = "SELECT * FROM {0} WHERE id_post=={1} ORDER BY dtime DESC".format(self.table_stats, postid)
        for row in self.execute (sql) :
            nb -= 1
            if nb < 0 : break
    ...
    def enumerate_posts (self,
        ...
        """
        self.connect()
        sql = self.private_process_condition(blog_selection, post_selection, sorted, specific, now, searchterm)
        sql += " LIMIT %d" % first
