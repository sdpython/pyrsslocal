...
class XMLHandlerDictNode (dict) :
    ...
    def set (self, i, v) :
        ...
        if i in self :
            if isinstance (v, XMLHandlerDictNode) :
                ...
                raise Exception ("unable to append a new string value for an existing field %s:%s" % (i,v))
        else :
            self [i] = v
        return self
    ...
    def is_text_only (self) :
        ...
        """
        if len (self.other) > 0 :   return False
    ...
    def get_xml_output (self) :
        ...
        """
        att = [ "" ] + [ "%s=\"%s\"" % (k, escape (self [k])) for k in sorted (self) if len (self [k]) <= 20 ]
        att = " ".join (att)
        ...
        if len (self.other) == 0 :
            if len (self.buffer) == 0 :
                ...
                return "%s<%s%s>%s</%s>\n" % (lev, self.name, att, self.buffer, self.name )
        else :
            res = [ "%s<%s%s>\n" % (lev, self.name, att) ]
            if len (self.buffer) > 0 :
    ...
    def get_values_group (self, fields, nb = 1) :
        ...
        if len (res) >= nb :
            ...
            for k,v in res :
                if k in temp :
                    raise Exception ("field %s already present in '%s' (full name '%s')" % (k, ", ".join (temp.keys ()), "/".join (self.get_full_name ())))
                temp [k] = v
            for f in fields :
                if f not in temp : temp [f] = None
    ...
    def iadd (self, other, use_list, collapse) :
        ...
        for node in other.other :
            ...
            if not ok :
                ...
                if use_list : nt[1]._convert_into_list ()
        ...
        for k,v in self.other :
            ...
            else :  count [k,1] = count.get ( (k,1), 0) + 1
    ...
    def _check_ (self, add_root_id) :
        ...
        if "_othercount" not in self.__dict__ :
            fLOG (self)
            raise Exception ("unable to find _othercount in '%s'" % "/".join (self.get_full_name ()))
        for k,v in self.other :
            count [k] = count.get (k,0) + 1
        if len (count) > 0 :
            if max (count.values ()) > 1 :
                fLOG (self)
                raise Exception ("max (count.values ()) > 1 in '%s' \nexp: %s" % ("/".join (self.get_full_name ()), str (count)))
        for k,v in self.other :
    ...
    def _adopt_table (self, tbl, exception) :
        ...
        for k,v in count.items () :
            if k not in tbl._othercount :
                fLOG ("ERROR: unable to find field '%s' (1:n) in path '%s'" % (k, "/".join (self.get_full_name ())))
                self._log_error ()
                if exception : raise Exception ("unable to find field '%s' (1:n) in path '%s'" % (k, "/".join (self.get_full_name ())))
    ...
    def _transfer_to_object (self, root = True, exception = True) :
        ...
        try :
            ...
            if "conversion_table" not in self.__dict__ :
                fLOG ("ERROR: unable to find conversion_table for field ", self.name, " in node " + "/".join (self.get_full_name ()))
                self._log_error ()
                #if exception : raise Exception ("fail to convert value for field " + k)
            elif len (self.buffer) > 0 :
                fLOG ("ERROR: fail to convert value '", self.buffer, "' into ", self.conversion_table.get (self.name, "not found"), " for field ", self.name, " --- ", repr (self.buffer), " path: ", "/".join (self.get_full_name ()))
                self._log_error ()
        ...
        for k,v in self.items () :
            try :
                ...
                if "conversion_table" not in self.__dict__ :
                    fLOG ("ERROR: unable to find conversion_table field " + k + " in node " + "/".join (self.get_full_name ()))
                    self._log_error ()
                    #if exception : raise Exception ("fail to convert value for field " + k)
                    continue
                elif len (v) > 0 :
                    fLOG ("ERROR: fail to convert value ", v, " field ", k, " into ", self.conversion_table.get (k, "not found"), " for field ", "/".join (self.get_full_name ()))
                    self._log_error ()
                    if exception : raise Exception ("fail to convert value for field '%s' in node '%s'" % (k, "/".join (self.get_full_name ())))
                    else : continue
                else :
                    continue
