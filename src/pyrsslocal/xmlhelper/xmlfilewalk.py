...
def _iteration_values (values) :
    ...
    co = [ ]
    for v in values :
        if isinstance (v, list) : co.append (v)
        else : co.append ( [v] )
...
def table_extraction_from_xml_files_iterator (file, fields, log = False) :
    ...
    if log : fLOG ("table_extraction_from_xml_files: begin")
    ...
    for i_,o in enumerate (parser.parse (fileh)) :
        ...
        for look,typ in fields :
            ...
            if typ == "one" :
                if len (path) == 0 :
                    ...
                    raise Exception ("unable to find a value for path %s" % "/".join (look))
                val = path [0]
                if val == None : val = ""
            elif typ == "all" :
                if len (path) == 1 : val = path [0]
                elif len (path) == 0 : val = ""
                else :
                    val = path
                    nb += 1
            else :
                raise Exception ("the type must in (one, all) %s,%s" % (look, typ))
            values.append (val)
        ...
        if log and (i_+1) % 1000 == 0 : fLOG ("table_extraction_from_xml_files reading ", i_)

    if isinstance (file, str) : fileh.close ()
    if log : fLOG ("table_extraction_from_xml_files: end")
...
def table_extraction_from_xml_files (file, output, fields, log = False) :
    ...
    @param      log         do logs if True
    """
    outputh = open (output, "w", encoding="utf8") if isinstance (output, str) else output
    for line in table_extraction_from_xml_files_iterator (file, fields, log) :
        outputh.write (line)
        outputh.write (GetSepLine ())
    if isinstance (output, str) : outputh.close ()
...
def xml_filter_iterator (file, filter, log = False, xmlformat = True) :
    ...
    if isinstance (file, str) : fileh.close ()
    if log : fLOG ("xml_filter_iterator: end")
